...
class Ability:
    ...
    def update(self, time, damage_done, shots_fired):
            if not time == 0: 
                print("| shot " + str(shots_fired) +"| time: "+str(format(time,".2f")) + "| damage: " + str(int(float(format(damage_done, ".0f")))) + "| dps: " + str(format((damage_done)/time,".0f"))) 
            else:
                print("| shot " + str(shots_fired) +"| time: "+str(format(time,".2f")) + "| damage: " + str(int(format(damage_done, ".0f"))) + "| dps: infinity") 
...
class ChaosReach(Ability):
    ...
    def printDps(self, name = "Chaos Reach", damageTimes = [], placeInColumn = None):
        ...
        for i in range(ticks):
            ...
            self.damage_times.append(self.update(self.time, self.damage_done, ticks))    
            self.time+=1/6
        return self.excel.closeExcel(self.damage_times)  
