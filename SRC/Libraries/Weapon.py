...
class Weapon:
    ...
    def processSimpleDamageLoop(self, inital_mag, subsequent_mag, timeBetweenShots, reloadTime, damagePerShotFunction):
        ...
        while shots_fired < self.reserves and self.time < 100:
            ...
            mag += 1
    def processBaitDamageLoop(self, bait_tuple, inital_mag, subsequent_mag, timeBetweenShots, reloadTime, damagePerShotFunction, bait_duration = 10):
        mag_size = inital_mag
        shots_fired = 0
        def procBait():
            print(f"---------------------") 
        ...
        while shots_fired < self.reserves and self.time < 100:
            for shots_fired_this_mag in range(mag_size):
                if self.time > bait_time + bait_duration and shots_fired != mag_size - 1 and shots_fired != self.reserves - 1:
